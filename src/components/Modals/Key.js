...
const Key = ({ isOpen ,handleClose })=>{
  const [state, dispatch] = useContext(GraphContext);
  const classes = useStyles();

  const [dp, setdp] = React.useState("");
  const [currentKeyName, setCurrentKeyName] = useState("key0");
  ...
  const handleAddNode = () => {
    ...
    dispatch({
      type: "ADD_KEY",
      payload: { id: currentKeyName, color: "yellow" },
    });
    if (linkDest) {
      dispatch({
        type: "ADD_LINK",
        payload: {
          source: currentKeyName,
          target: linkDest,
          label: linkWeight,
        },
      });
    ...
    setLinkDest("");
    setdp("");
    setCurrentKeyName(`source${state.nodes.length + 1}`);
  };
